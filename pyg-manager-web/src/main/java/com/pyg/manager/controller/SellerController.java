...
public class SellerController {
	...
	/**
	 * 需求：商家审核 （修改商家状态）
	 * 参数：String status,TbSeller seller
	 */
	@RequestMapping("updateSellerStatus/{status}")
	public PygResult updateSellerStatus(@PathVariable String status,@RequestBody TbSeller seller){
		try {
			//调用远程服务方法，实现商家审核
			sellerService.updateSellerStatus(status,seller);
			//修改成功
			return new PygResult(true,"修改成功");
		} catch (Exception e) {
			e.printStackTrace();
